...
def beam_weights_hypercardioid(order: int) -> np.ndarray:
    ...
    * Rafaely, B. (2015), Eq. (4.26).
    ...
    --------
    >>> import numpy as np
    >>> from spherical_array_processing.beamforming import beam_weights_hypercardioid, axisymmetric_pattern
...
def beam_weights_maxev(order: int) -> np.ndarray:
    ...
    * Zotter, F. & Frank, M. (2019). *Ambisonics*. Springer.  Sec. 4.3.2.
    ...
    --------
    >>> import numpy as np
    >>> from spherical_array_processing.beamforming import beam_weights_maxev, axisymmetric_pattern
