...
Usage:
    from spharray.regression.matlab import <symbol>
"""
...
def probe_matlab_cli(timeout_s: int = 15) -> MatlabProbeResult:
    ...
    cmd = [rt.executable, "-nojvm", "-nodisplay", "-nosplash", "-r", "disp('SPHARRAY_MATLAB_PROBE_OK');exit"]
    try:
    ...
    if "spharray_matlab_probe_ok" in text and cp.returncode == 0:
